...
internal class PackageValidator(context: Context, @XmlRes xmlResId: Int) {
    ...
    private fun logUnknownCaller(callerPackageInfo: CallerPackageInfo) {
        if (callerPackageInfo.signature != null) {
            Log.i(TAG, "Unknown caller: ${callerPackageInfo.name} (package: ${callerPackageInfo.packageName}, signature=$callerPackageInfo.signature)")
        }
    }
    ...
    private fun buildCallerInfo(callingPackage: String): CallerPackageInfo? {
        ...
        val appName = packageInfo.applicationInfo
            ?.loadLabel(packageManager)?.toString() ?: return null
        ...
        if (requestedPermissions != null && permissionFlags != null) {
